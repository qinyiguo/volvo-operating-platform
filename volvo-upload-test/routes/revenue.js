...
// ── 營收目標 Excel 匯入（原生格式）──
router.post('/upload-revenue-targets-native', requirePermission('feature:upload_targets'), upload.single('file'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: '請選擇檔案' });
...
// 提交本週預估（每週每據點只能提交一次）
router.post('/revenue-estimates/weekly-submit', requirePermission('feature:revenue_target_edit'), async (req, res) => {
