...
// ══ 執行匹配（對全部 pending 記錄）══
// 匹配邏輯：
// 1. 依 plate_no_norm + branch 查 business_query，repair_type 含鈑/保險/事故
// 2. 找到工單後，查 repair_income work_order+branch，clear_date IS NOT NULL 為結清
// 3. 結清 → status='settled', 計算獎金；未結清 → status='matched_pending'
router.post('/bodyshop-bonus/match', async (req, res) => {
  const { period } = req.body; // 指定結算期間
  if (!period) return res.status(400).json({ error: 'period 為必填' });

  try {
    // 取得所有待匹配記錄（pending + matched_pending）
    ...
    for (const app of appRows) {
      ...
      const bqRes = await pool.query(
        `SELECT work_order, branch, repair_type, settle_date, open_time
         FROM business_query WHERE ${bqConds.join(' AND ')}
      ...
      // ── Step 2: 對每個工單查 repair_income 是否結清 ──
...
router.get('/bodyshop-bonus/summary', async (req, res) => {
  ...
  try {
    const conds = [`status='settled'`, `settled_period=$1`];
    const params = [settled_period];
    if (branch) { conds.push(`branch=$2`); params.push(branch); }
    ...
    const r = await pool.query(`
      SELECT
        ...
        array_agg(plate_no_raw ORDER BY apply_date) AS plates,
        array_agg(work_order  ORDER BY apply_date) AS work_orders,
        array_agg(bonus_amount ORDER BY apply_date) AS bonuses,
        array_agg(income_total ORDER BY apply_date) AS incomes,
        array_agg(bonus_rate  ORDER BY apply_date) AS rates,
        array_agg(settle_date::text ORDER BY apply_date) AS settle_dates
    ...
    const pendingRes = await pool.query(`
      ...
      FROM bodyshop_bonus_applications
      WHERE status IN ('matched_pending','pending') AND (
        app_period <= $1
      )
      GROUP BY applicant_name, emp_id
    `, [settled_period]);
    const pendingMap = {};
    pendingRes.rows.forEach(p => { pendingMap[p.emp_id || p.applicant_name] = parseInt(p.pending_count); });
