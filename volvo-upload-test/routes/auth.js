...
router.post('/auth/settings', async (req, res) => {
  ...
  const stored = await getSettingsPassword();
  if (stored == null) return res.status(503).json({ error:'系統尚未初始化' });
  if (password !== stored) return res.status(401).json({ error:'密碼錯誤' });
...
router.put('/auth/settings/password', async (req, res) => {
  ...
  const stored = await getSettingsPassword();
  if (stored == null) return res.status(503).json({ error:'系統尚未初始化' });
  if (currentPassword !== stored) return res.status(401).json({ error:'目前密碼不正確' });
