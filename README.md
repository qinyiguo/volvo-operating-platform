...
3. **寫入類端點**記得加 `requirePermission('feature:bonus_edit')` 等。
4. 若要記 audit log 以外的特殊事件,在 handler 內 `await writeLog(req, {action:'XXX', ...})`。
5. 前端直接 `fetch('/api/...')`,`auth.js` 的全域 wrapper 會自動帶 token。
